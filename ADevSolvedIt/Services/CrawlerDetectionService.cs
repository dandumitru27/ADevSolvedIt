...
{
    public static class CrawlerDetectionService
    {
        ...
        public static bool IsCrawler(string userAgent)
        {
            return Regex.IsMatch(
                userAgent,
                @"bot|crawler|spider|80legs|ia_archiver|voyager|curl|wget|yahoo! slurp|mediapartners-google|facebookexternalhit|http-client|node-fetch",
                RegexOptions.IgnoreCase
            );
        }

        public static bool IsCrawlerAdvanced(string userAgent)
        {
            string userAgentLower = userAgent.ToLower();

            return Crawlers.Exists(x => userAgentLower.Contains(x));
        }
    }
}
